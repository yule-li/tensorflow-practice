...
CMD="python train/train_timeline.py --logs_base_dir logs/${NAME}/ --models_base_dir models/$NAME/ --data_dir dataset/CASIA-WebFace-112X96 --model_def models.inception_resnet_v1  --optimizer MOM --learning_rate -1 --max_nrof_epochs ${MAX_EP} --random_flip --learning_rate_schedule_file ${LR_FILE}  --num_gpus 1 --weight_decay ${WEIGHT_DECAY} --loss_type ${LOSS_TYPE}  --network ${NETWORK} --epoch_size 10 --max_nrof_epochs 1 "
echo Run "$ARGS ${CMD}"
eval "$ARGS ${CMD}"
